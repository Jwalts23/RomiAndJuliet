...
public:
    ...
    void IMUsetup(void);
    float IRloop(float targetDistanceIR);
    float IRloop2(float targetDistanceIR);
    void IRsetup();
    void PID();
    void WallFollowIR(float d);
    void MaintainDistance(float d);
    ...
    enum States{start,driveFlat,onRamp,StopAtTop,DownTheRamp,DriveAfterTipping,NextPositionForScene,turn,driveStraight,turn2,START_Blinking,STOP};
    States state=start;
    float irEffort;
